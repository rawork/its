...
//	   }
		...
		while( (ord($ws[$ptr])!=0x0a) && ($ptr<strlen($ws)) ) {
		 switch (ord($ws[$ptr])+256*ord($ws[$ptr+1])) {
		  ...
		  case 0x0204:
			...
			$data['cell'][$row][$col]['data'] = sizeof($this->sst['data'])-1;
	...
	function parse_workbook() {
		...
		if( sizeof($this->fat) < $num_fat_blocks )
			return 6;
		...
		if( $small_block != 0xfeffffff )  {
		 ...
		 $this->max_sblocks = sizeof($small_data_chain) * 8;

		 $schain = $this->get_blocks_chain($small_block);
		 for( $i=0; $i<sizeof($schain); $i++ ) {
		 ...
		 $this->sfc = '';
		 for( $i=0; $i<sizeof($small_data_chain); $i++ )
